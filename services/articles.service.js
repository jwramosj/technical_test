			...
			handler(ctx) {
				...
				return this.validateEntity(entity)
					.then(() => {
						...
						entity.slug = createSlug(entity.title) + "-" + Math.floor(Math.random() * Math.pow(36, 6)).toString(36);
	...
	methods: {
		...
		transformResult(ctx, entities, user) {
			if (Array.isArray(entities)) {
				return Promise.all(entities.map(item => this.transformEntity(ctx, item, user)))
